...
There is likely multicollinearity happening. The features are highly correlated with each other.
...
#correlation
attach(College)
college = data.frame(log.Enroll,Expend,log.Accept,log.P.Undergrad,perc.alumni,Personal)
...
lambdalist = (0:10)/100  # order large to small
plot(lambdalist)
#fit ridge regression - need alpha = 0
#RRfit = glmnet(x, y, alpha = 0,lambda=lambdalist)
#coef(RRfit,s=0.1)
...
college = data.frame(log.Enroll,Private,Top10perc,Top25perc,Outstate,Room.Board,Books,Personal,PhD,Terminal,S.F.Ratio,perc.alumni,Expend,Grad.Rate,log.Apps,log.Accept,log.F.Undergrad,log.P.Undergrad)
...
ncv = 10
n = dim(college)[1]
groups = c(rep(1:10,77),(1:7)); cvgroups = sample(groups,n)

library(glmnet)
lambdalist = (1000:1)/1000  # order large to small
plot(lambdalist)
#ENET alpha=0.5 cross-validation
cvENET50glm = cv.glmnet(x, y, lambda=lambdalist, alpha = 0.75, nfolds=ncv, foldid=cvgroups)
bestlambdaENET50 = lambdalist[whichlowestcvENET50]; bestlambdaENET50;
whichlowestcvENET50 = order(cvENET50glm$cvm)[1]; min(cvENET50glm$cvm)
{
  plot(cvENET50glm$lambda,cvENET50glm$cvm,type="l",lwd=2,col="red",xlab="lambda",ylab="CV(10)", xlim=c(0,.02),ylim = c(0,.1))
  bestlambdaENET50 = lambdalist[whichlowestcvENET50]; bestlambdaENET50; abline(v=bestlambdaENET50)
}
#fit selected model
Bestfit = glmnet(x, y, alpha = 0.75,lambda=lambdalist)

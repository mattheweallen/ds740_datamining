...
<span style="color:green">**Numeric Answer**  0.1700935</span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
MMClass = predict(fit, OJ, type = "class")
confusion = table(predicted=MMClass, actual=OJ$Purchase)
...
# 
# 
# DefaultClass.8 = rep(NA, length(OJ$Purchase))
# DefaultClass.8[which(fit$fitted.values > .5)] = "MM"
# DefaultClass.8[which(fit$fitted.values < .5)] = "CH"
# 
# confusion = table(predvals = DefaultClass.8, truth = OJ$Purchase)
# (confusion[1,2] + confusion[2,1])/sum(confusion)
# 
...
<span style="color:green">**Numeric Answer**  0.04925054</span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
PurchaseMM.9 = rep(NA, length(OJ$Purchase))
PurchaseMM.9[which(fit$fitted.values > .9)] = "MM"
PurchaseMM.9[which(fit$fitted.values < .1)] = "CH"

confusion = table(predvals = PurchaseMM.9, truth = OJ$Purchase)
(confusion[1,2] + confusion[2,1])/sum(confusion)

#length(which(is.na(PurchaseMM.9)))
```
...
<span style="color:green">**Numeric Answer** 23 </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
14+9
```
...
library(nnet)
data("Hitters")
Hitters = Hitters[which(!is.na(Hitters$Salary)),] #remove rows where salary is na

#add columns League01, Division01, NewLeague01
Hitters$League01 = rep(0,length(Hitters$League))
Hitters$League01[which(Hitters$League == "N")] = 1 

Hitters$Division01 = rep(0,length(Hitters$Division))
Hitters$Division01[which(Hitters$Division == "W")] = 1

Hitters$NewLeague01 = rep(0,length(Hitters$NewLeague))
Hitters$NewLeague01[which(Hitters$NewLeague == "N")] = 1

#remove columns League, Division, NewLeague
...
for(i in 1:k){
  groupi = (cvgroups == i)
  myHitters.train = scale(Hitters[!groupi,])
  myHitters.valid = scale(Hitters[groupi,], center = attr(myHitters.train, "scaled:center"), 
  scale = attr(myHitters.train, "scaled:scale"))
  
  for(j in 1:length(decayRate)){
    fit = nnet(Salary ~ ., data = myHitters.train, size = 10, maxit = 1000, linout = T, trace = F, decay =decayRate[j])
    squaredError[groupi, j] = (myHitters.valid[ , 17] - predict(fit, myHitters.valid) )^2
  } # end iteration over j
} # end iteration over i

MSE = apply(squaredError, 2, mean)
plot(decayRate,MSE,type="l",lwd=2,las=1)
min(MSE)
which(MSE == min(MSE))
decayRate[which(MSE == min(MSE))]




set.seed(10)
Hitters.std <- scale(Hitters)
fit <- nnet(Salary ~ ., 
            data=Hitters.std,
            size=10,
            decay=1.5,
...
<span style="color:green">**Text Answer**: The value 1.5 for lambda minimizes the MSE. Lambda equal to 1.4 or 1.3 might also be a good choice, because they are close to the minimum.</span>
...
<span style="color:green">**Text Answer**: AtBat and Years are the two most important variables for predicting standardized Salary.</span>
...
<span style="color:green">**Text Answer**:One thing to do would be to decrease the number of inputs to the model. Use the top 5 predictors based on Garson's algorithm for example. Another thing to do would be to find an optimal number of hidden layers through cross validation.</span>

...
# misclassError = rep(NA, k)
# conv = rep(NA, k) 
# set.seed(10)
# cvgroups = sample(groups,n) 
# for(i in 1:k){
#     groupi = (cvgroups == i)
#     myHitters.train = scale(Hitters[!groupi, -17])
#     myHitters.valid = scale(Hitters[groupi, -17], center = attr(myHitters.train, "scaled:center"), 
#             scale = attr(myHitters.train, "scaled:scale"))
#     myHitters.train = data.frame(Salary=Hitters[!groupi, 17], myHitters.train)
#     myHitters.valid = data.frame(Salary=Hitters[groupi, 17], myHitters.valid)
#     
#     fit = nnet(Salary ~ ., data=myHitters.train, size = 10, linout=T, maxit = 1000) 
#     predictions = predict(fit, myHitters.valid)
#     misclassError[i] = length(which(predictions != myHitters.valid[ , 17])) / length(predictions)
...
set.seed(10)
cvgroups = sample(groups,n) 
squaredError = matrix( , nr = k, nc = length(decayRate) ) 
...
for(i in 1:k){
  ...
  for(j in 1:length(decayRate)){
    ...
    squaredError[groupi, j] = (Hitters$Salary[groupi] - predict(fit, myHitters.valid) )^2
...
#   
#   for(j in 1:length(decayRate)){
#     fit = nnet(Salary ~ ., data = Hitters[!groupi,], size = 10, maxit = 1000, linout = T, trace = F, decay =decayRate[j])
#     
#     squaredError[groupi, j] = (Hitters$Salary[groupi] - predict(fit, Hitters[groupi,]) )^2
#   } # end iteration over j
# } # end iteration over i

...
#library(nlme)
data("crime2005")
m1 = lm(VI2~ME + PO, data = crime2005)
...
The states Delaware, South Carolina, and District of Columbia appear to be outliers.
...
while(sum(abs(oldcoef-newcoef))>.0001 & iter < 100) {
  ...
  k = 1.345*MAR/0.6745 #tuning parameter k=1.345*sigma for Huber
...
#VI2 = -40.3926844 + 0.6794144*ME + 3.2950743*PO
...
```{r, echo=TRUE}
library(MASS)
fit.bisquare = rlm(VI2~ME + PO, data = crime2005, psi = psi.bisquare)
fit.bisquare$coefficients
```
...
<span style="color:green">**Text Answer**: #VI2 = -31.2925739 + 0.6113925*ME + 2.8080861*PO</span>
...
#VI2 = -40.3926844 + 0.6794144*ME + 3.2950743*PO, HUBER
#VI2 = -31.2925739 + 0.6113925*ME + 2.8080861*PO, BISQUARE

<span style="color:green">**Text Answer**: </span>
Tukey's bisqure is less influenced by the outliers as compared to Huber.
...

{
  ...
  showLabels(1:dim(crime2005)[1], fit.bisquare$w, crime2005$STATE, method = smallweights)
}
...
<span style="color:green">**Code Answer**: </span>
```{r, echo=TRUE}
elnino = na.omit(read.csv("elnino.csv"))
```
...
The linear model is not appropriate here, because it is clear from the plots that the residuals are not normally distributed which is an underlying assumption.
...
```{r,echo=FALSE}
#colnames(elnino)
model = air.temp~zon.winds+mer.winds+humidity+s.s.temp
fit.lm = lm(model, data=elnino)
plot(fit.lm)
fit.lm

library(nlme)
fit.gls = gls(model, data=elnino)
fit.gls
summary(fit.gls)
```
...
They are essentially the same. Both models do not take into account correlated errors. The coefficients are computed in two different ways, and they agree with each other. The lm model uses Ordinary Least Squares and the gls model uses Restricted Maximum Likelihood.
...
<span style="color:green">**Code Answer**: </span>
```{r, echo=TRUE}
buoy3.resids = fit.gls$residuals[which(elnino$buoy == 3)]
plot(buoy3.resids[-1]~buoy3.resids[-14], xlab="Rediduals Previous Day", ylab="Rediduals")
```
...
It does appear that there is autocorrelation with the residuals for buoy 3, because there appears to be an increasing trend in the residuals.
...
<span style="color:green">**Code Answer**: </span>
```{r, echo=TRUE}
fit.auto = gls(model, data=elnino, correlation = corAR1(form = ~1 | buoy))
summary(fit.auto)

fit.auto$coefficients
...
Since measurements at each buoy are strongly correlated with measurements from the previous day, an autoregressive model of order 1 seems to be more appropriate. The first model assumes uncorrelated error terms. From the plot of the error with the previous day's error for buoy 3, there is evidence that successive days errors are indeed correlated with each other.
<span style="color:green">**Text Answer**: </span>
...
The AIC for model C of 381.2676 is significantly better than for model B of 694.1104. They are not reasonable alternatives. AIC takes into account the fit and the number of parameters, so model C is a better tradeoff between fit and number of parameters.
...
```{r,echo=FALSE}
covMatrix = getVarCov(fit.auto) #grab the first off diagnol number. this is the correlation between noise terms of consecutive days. divide by variance which is on the diagnol
covMatrix[1,2]/covMatrix[1,1]
```

...
<span style="color:green">**Text Answer**: There is significant variability of the standard deviations and means between the characteristics. It is a good idea to standardize before clustering, so that characteristics are compared on equal terms.</span>
...
```{r,echo=FALSE}
x.scale.distance = dist(x.scale, method="euclidean")
hc.complete = hclust(x.scale.distance,method="complete")
...
<span style="color:green">**Numeric Answer**  7.5</span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
hc.model$height
#examine dendogram

hc.single = hclust(x.scale.distance,method="single")
hc.average = hclust(x.scale.distance,method="average")
...
<span style="color:green">**Text Answer**: The dendograms for single linkage and average linkage are both highly asymmetric. Average linkage looks like there are three groups, but as you go further up the tree the more unevenly distributed it becomes. Complete linkage shows a clear 3 groups that look symmetrically distributed.</span>
...
<span style="color:green">**Graph Answer**  </span>: 
  (post to discussion board on D2L)
```{r,echo=FALSE}
memb = cutree(hc.complete,k=3)
with(x, plot(Alcohol ~ Dilution, col=memb, pch=memb))
```
...
par(mfrow=c(1,3))
with(x, plot(Alcohol ~ Dilution, col=memb1, pch=memb1))
with(x, plot(Alcohol ~ Dilution, col=memb2, pch=memb2))
with(x, plot(Alcohol ~ Dilution, col=memb3, pch=memb3))
...
make_cluster_graph(x.scale,"Dilution","Alcohol",5)
make_cluster_graph(x.scale,"Dilution","Alcohol",4)
make_cluster_graph(x.scale,"Dilution","Alcohol",3)
make_cluster_graph(x.scale,"Dilution","Alcohol",2)
```
...
<span style="color:green">**Numeric Answer** 62 </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
set.seed(12)
...
<span style="color:green">**Numeric Answer** 65 </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
```
...
<span style="color:green">**Numeric Answer**  51</span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
```{r,echo=FALSE}
```

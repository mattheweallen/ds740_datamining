...
Number of domestic vehicles that are correctly classified = 204</span>
...
myroc$auc
plot.roc(myroc, main="ROC of domestic car predicted by displacement")
```
...
Displacement has a significantly larger area under the ROC curve than does mpg (0.9267 vs 0.8407). Based purely on AUC, displacement should be selected.
...
#todo should displacement have been used instead of weight for problem 1?
<span style="color:green">**Graph Answer**  </span>: 
(post to discussion board on D2L)
```{r,echo=FALSE}
require(ISLR)
...
Auto$Colour[Auto$origin==3]="red"
Auto$Colour[Auto$origin==1]="blue"
# Plot all points at once, using newly generated colours
{
plot(Auto$mpg~Auto$displacement, col=Auto$Colour, xlab="displacement", ylab = "mpg", pch=16)
legend("topright", legend=c("American","European","Japanese"), pch=16, col=c("blue","black","red"))
}
```
...
library(MASS)
attach(Auto)
ldafit = lda(origin~mpg+cylinders+displacement+horsepower+weight)
ldafit
#Question 14, 20.03347
#Question 15, 27.60294
#Question 16, 30.45063
```

Mean mpg for American vehicles = 20.03347  
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  

Mean mpg for European vehicles = 27.60294 
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  

Mean mpg for Japanese vehicles = 30.45063  
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
...
```{r,echo=FALSE}
fittedclass = predict(ldafit,data=Auto)$class
table(Auto$origin,fittedclass)
diag(table(Auto$origin,fittedclass))

n = dim(Auto)[1]
ErrorLDA = sum(origin != fittedclass)/n
ErrorLDA
...
```{r,echo=FALSE}
#Auto$mpg+Auto$cylinders+Auto$displacement+Auto$horsepower+Auto$weight
ldafit
newdata = data.frame(mpg = 20, cylinders = 8, displacement = 320, horsepower = 280, weight = 3600)
predict(ldafit,newdata)$class
```
...
```{r,echo=FALSE}
qdafit = qda(origin~mpg+cylinders+displacement+horsepower+weight)
qdafit

fittedclassQDA = predict(qdafit,data=Auto)$class
table(Auto$origin,fittedclassQDA)
diag(table(Auto$origin,fittedclassQDA))

n = dim(Auto)[1]
ErrorQDA = sum(origin != fittedclassQDA)/n
ErrorQDA
#Question 21
#Question 22
#Question 23
...
Both have a high error rate, but the QDA fit overall is a little better. It is significantly better at predicting Japanese origin.
...
#todo look at slide
#https://piazza.com/class/jptvgleimyv268?cid=86

```{r,echo=FALSE}
#Question 26
K=3; p=1; K+K*p+p*(p+1)/2
#Question 27
K=3; p=2; K+K*p+p*(p+1)/2
#Question 28
K=3; p=5; K+K*p+p*(p+1)/2
#Question 29
K=3; p=1; K+K*p+K*p*(p+1)/2
#Question 30
K=3; p=2; K+K*p+K*p*(p+1)/2
#Question 31
K=3; p=5; K+K*p+K*p*(p+1)/2
```


Model 1: # of parameters =  7
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  

Model 2: # of parameters = 12 
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  

Model 3: # of parameters = 33  
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  

Model 4: # of parameters = 9  
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  


Model 5: # of parameters = 18  
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  

Model 6: # of parameters = 63  
<span style="color:green">**Numeric Answer**  </span> 
**<span style="color:red">(AUTOGRADED on D2L)</span>**:  
...
for (i in 1:cvk)  {
  ...
  newdata3 = data.frame(Auto[cvgroups==i,c(1,2,3,4,5)])
...
for (i in 1:cvk)  {
  ...
  newdata1 = data.frame(Auto[cvgroups==i,3])
...
Model 3 has fewer parameters to estimate and would be computationally less expensive.
...
There appears to be strong relationships between between many of the variables.

```{r, echo=TRUE}
pairs(Auto[,c(1,2,3,4,5)])
```
...
xDomesitc = xvar[origin == 1,]
xEuropean = xvar[origin == 2,]
xJapanese = xvar[origin == 3,]

#install.packages("MVN")
library(MVN)
mvn(xDomesitc, mvnTest = "hz")$multivariateNormality
mvn(xEuropean, mvnTest = "hz")$multivariateNormality
mvn(xJapanese, mvnTest = "hz")$multivariateNormality
...
The Henze-Zirkler test concludes that the assumption of multivariate normality is violated. Since the assumption is not met, the QDA model is not appropriate.
...
knn might be a good choice here, because it does not make any assumptions about the underlying distribution.